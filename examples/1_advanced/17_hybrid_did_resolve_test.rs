...
async fn main() -> anyhow::Result<()> {
  ...
  let (_, doc, _) = create_did(&client, &mut secret_manager_issuer, &storage_issuer, CompositeAlgId::IdMldsa44Ed25519Sha512).await?;
  ...
  let (_, doc, _) = create_did(&client, &mut secret_manager_issuer, &storage_issuer, CompositeAlgId::IdMldsa65Ed25519Sha512).await?;
