...
async fn create_did_zk(client: &Client, address: Address, network_name: &NetworkName, secret_manager: &SecretManager, storage: &MemStorage, key_type: KeyType, alg: ProofAlgorithm) -> anyhow::Result<(Address, IotaDocument, String)> {
  ...
  async fn main() -> anyhow::Result<()> {
    ...
    for _ in 0..n {
      let (_, doc, _) = create_did_zk(&client, address, &network_name, &secret_manager_issuer, &storage_issuer, JwkMemStore::BLS12381G2_KEY_TYPE, ProofAlgorithm::BLS12381_SHA256).await?;
      if printing {
    ...
    for _ in 0..n {
      let (_, doc, _) = create_did_zk(&client, address, &network_name, &secret_manager_issuer, &storage_issuer, JwkMemStore::BLS12381G2_KEY_TYPE, ProofAlgorithm::BLS12381_SHAKE256).await?;
      if printing {
        let a = doc.to_json_vec()?;
        println!("{}", a.len());
      }
